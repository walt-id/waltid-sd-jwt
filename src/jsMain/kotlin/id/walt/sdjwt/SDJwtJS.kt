...
class SDJwtJS(
  ...
  companion object {
    @OptIn(DelicateCoroutinesApi::class)
    fun verifyAndParseAsync(sdJwt: String, jwtCryptoProvider: JSAsyncJWTCryptoProvider): Promise<VerificationResult<SDJwtJS>> = GlobalScope.promise {
    ...
    @OptIn(DelicateCoroutinesApi::class)
    fun signAsync(sdPayload: SDPayload, jwtCryptoProvider: JSAsyncJWTCryptoProvider, keyID: String? = null, withHolderJwt: String? = null): Promise<SDJwtJS> = GlobalScope.promise {
      SDJwtJS(
